...
impl StudioLlmProcessor {
    ...
    pub async fn decompose_script(&self, script: &str) -> Result<Vec<ScriptItem>> {
        info!("Decomposing script for visual keywords using {}", self.provider_or_model);

        let system_prompt = r#"You are an AI video producer. Break down the user's script into individual sentences or natural thought chunks.
...
[
  ...
  {"sentence": "Black holes consume everything in their path.", "keyword": "black hole space"}
    ...
    pub async fn generate_topic_script(&self, topic: &str, vibe: &str) -> Result<String> {
        info!("Generating script for topic '{}' with vibe '{}'", topic, vibe);

        let prompt = format!(
            "Write an engaging 45-60 second faceless short video script on the topic: '{}'. Vibe: {}. Keep sentences punchy, hook-driven, and entertaining. Plain text script only.",
            topic, vibe
        );
        ...
        if self.provider_or_model.contains('/') || !self.openrouter_key.is_empty() {
            ...
            let model = if self.provider_or_model.is_empty() { "gemini-3.1-flash-lite" } else { &self.provider_or_model };
            let url = format!(
